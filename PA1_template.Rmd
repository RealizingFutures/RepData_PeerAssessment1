...
activity.spd.median
```
The mean total number of steps taken per day is `r round(activity.spd.mean,0)` and the 
median is `r activity.spd.median`.
...
The maximum average time interval is at `r sub("(\\d+)(\\d{2})", "\\1:\\2", 
activity.spi[with(activity.spi, which.max(spi)), 1])` with `r 
round(activity.spi[with(activity.spi, which.max(spi)), 2], 0)` steps taken on
average.
...
There are `r rows.na` rows with missing values.  
...
The median of `r activity.im.median` is the same for the original data set and 
the data set with imputed values. However, the mean of the original data set is
`r round(activity.spd.mean, 0)` and the mean of the data set with imputed values is
`r round(activity.im.mean, 0)`. The mean is pulled up by replacing the NAs with 
